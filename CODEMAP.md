...
| View | `initCompact()` `openCompactForm(kind,ticker)` `renderCompactToday(s)` `compactRowMenu(ticker,watch)` | chuyển Gọn / Đầy đủ; dải **Việc cần làm hôm nay** chỉ tổng hợp tín hiệu đã/sắp đạt (`strategyAlert`), lệnh chờ và watchlist gần mục tiêu, không lưu gì thêm; thao tác ít dùng của từng mã (luận điểm, sửa giá kỳ vọng, bỏ theo dõi) nằm trong menu ba chấm. Mobile ≤768px bảng chuyển thành dòng thẻ hai tầng, desktop giữ dạng cột; sort/filter 2 bảng; chuyển form giao dịch hoặc thêm mã vào dialog và trả về khi đóng, giữ bản nháp. Bấm giá kỳ vọng dùng `editWatch`; `tx-submit` bị khoá trong lúc ghi để tránh gửi lặp |
